...
import { Store, Shield, Zap, CheckCircle2, Copy, ExternalLink } from 'lucide-react';
        ...
        {tabs.map(t => {
          ...
          return (
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5 text-xs sm:text-sm font-semibold rounded-xl transition-all ${tab === t.key ? 'bg-white dark:bg-gray-700 text-[#0A66FF] shadow-sm' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'}`}
            >
            ...
            return (
              <div key={plan.key} className={`bg-white dark:bg-gray-900 rounded-2xl p-4 shadow-sm flex items-center gap-4 ${isActive ? 'border-2 border-[#0A66FF]' : 'border-2 border-transparent'}`}>
                <div className="flex-1">
                  {isActive && (
                    <div className="flex items-center gap-1 text-[#0A66FF] text-xs font-bold mb-1">
                      <CheckCircle2 size={12} /> Plan actuel
                    </div>
                  )}
