...
import { LayoutDashboard, Package, ShoppingCart, Users, Settings, Zap, LogOut, ExternalLink, Moon, Sun, ChevronRight, Copy } from 'lucide-react';
...
const nav = [
  ...
  { label: 'Produits', href: '/dashboard/products', icon: <Package size={20} /> },
  { label: 'Commandes', href: '/dashboard/orders', icon: <ShoppingCart size={20} /> },
  { label: 'Clients', href: '/dashboard/customers', icon: <Users size={20} /> },
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${shop.plan === 'business' ? 'bg-purple-500/30 text-purple-300' : shop.plan === 'premium' ? 'bg-yellow-500/30 text-yellow-300' : 'bg-white/10 text-white/60'}`}>
                {shop.plan === 'business' ? 'Business' : shop.plan === 'premium' ? 'Premium' : 'Gratuit'}
              </span>
            </div>
          </div>
        ...
        {nav.map(item => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-semibold transition-all ${active ? 'bg-[#0A66FF] text-white shadow-lg shadow-blue-500/30' : 'text-white/60 hover:text-white hover:bg-white/10'}`}
            >
              {item.icon}
              {item.label}
